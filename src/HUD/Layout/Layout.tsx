...
export default class Layout extends React.Component<Props, State> {
    ...
    componentDidMount() {
        ...
        const handleViewType = (type: string) => {
            actions.on(type, () => {
                this.setState({
                    view: type === this.state.view ? null : (type as any),
                });
            });
        };
    ...
    render() {
                ...
                <div className="layout">
                    <div className={`draft-screen-container ${view === 'draft' ? '' : 'hide'}`}>
                        <div className="draft-container">
                            <TeamPicker
                                draft={game.draft.radiant}
                                type={'radiant'}
                                active={game.draft.activeteam === 2}
                            />
                            <div className="tournament_info">
                                <div className="bo shadowed-text">{(match && match.matchType) || 'BO2'}</div>
                                <div className="picker_and_logo">
                                    <div
                                        className={`side_pick left ${game.draft.activeteam === 2 ? 'active' : ''}`}
                                    ></div>
                                    <div
                                        className={`side_pick right ${game.draft.activeteam === 3 ? 'active' : ''}`}
                                    ></div>
                                </div>
                            </div>
                        ...
                        <div className="team_info_container s shadowed-text">
                            <TeamInfo
                                ...
                                type={'radiant'}
                                players={game.players.filter((player) => player.team_name === 'radiant')}
                            />
                            ...
                            <div className="timer">
                                ...
                                <div className="timer-time shadowed-text">
                            ...
                            <TeamInfo
                                ...
                                team={game.map.dire}
                                type={'dire'}
                                players={game.players.filter((player) => player.team_name === 'dire')}
                            />
                        </div>
                    </div>
        ...
        /> */}
                <TopSideBar
                    type="dire"
                    team={game.map.dire}
                    teamId={(game.map.dire && game.map.dire.id) || ''}
                    show={view === 'game'}
                >
